...
export function calculateByesNeeded(teamCount) {
  if (teamCount < 2) {
...
function generateRegularTournament(teams) {
  ...
  while (currentWinners > 1) {
    ...
    for (let i = 0; i < gamesInRound; i += 1) {
      nextRound.push([
        {
          name: `Winner ${i * 2 + 1}`,
          seed: i * 2 + 1,
          id: `winner-r${tournamentData.length}-g${i * 2}`
        },
        {
          name: `Winner ${i * 2 + 2}`,
          seed: i * 2 + 2,
          id: `winner-r${tournamentData.length}-g${i * 2 + 1}`
        }
      ]);
    }
...
export function generateTournamentWithByes(teams, strategy = "top-seeds") {
  if (teams.length < 2) {
    throw new Error("Tournament must have at least 2 teams");
  }
  ...
  while (currentWinners > 1) {
    ...
    if (currentWinners % 2 === 1) {
      nextRound.push([
        {
          name: `Winner ${currentWinners}`,
          seed: currentWinners,
          id: `winner-r${tournamentData.length}-g${currentWinners - 1}`
        }
      ]);
    }
