...
export function buildDefaultAvatarUrl(name) {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name || "Team")}&background=0F1726&color=E8EDF7&bold=true`;
}
...
export function getAvatarUrl(team) {
  if (!team) return "";
  const image = pickTeamImage(team);
...
function normalizeScheduleConfig(schedule) {
  ...
  return {
    startAt: String(schedule.startAt || schedule.start_at || "").trim(),
    startTime: String(schedule.startTime || schedule.start_time || "").trim(),
    timezoneOffset: String(
      schedule.timezoneOffset || schedule.timezone_offset || "+00:00"
    ).trim(),
    roundGapMinutes: normalizeMinuteValue(schedule.roundGapMinutes, 80),
    matchGapMinutes: normalizeMinuteValue(schedule.matchGapMinutes, 28),
    matchDurationMinutes: normalizeMinuteValue(schedule.matchDurationMinutes, 25)
  };
}
...
function buildDateStringForOffset(now, offset) {
...
function resolveScheduleStart(category, now) {
  const schedule = normalizeScheduleConfig(category?.schedule);

  if (schedule.startAt) {
    const exactStart = new Date(schedule.startAt);
    if (!Number.isNaN(exactStart.getTime())) {
      return exactStart;
    }
  }

  if (isValidTimeString(schedule.startTime)) {
    const dateString = buildDateStringForOffset(now, schedule.timezoneOffset);
    const dailyStart = new Date(
      `${dateString}T${schedule.startTime}:00${schedule.timezoneOffset}`
    );
...
export function normalizeCategory(category, index = 0, globalRoundLabelPresets = {}) {
  ...
  return {
    ...
    schedule: normalizeScheduleConfig(category?.schedule),
    teams: teams
...
export function buildScheduleMap(category, baseData, now = new Date()) {
  const schedule = normalizeScheduleConfig(category?.schedule);
  const baseStart = resolveScheduleStart(category, now);
  const roundGap = schedule.roundGapMinutes;
  const matchGap = schedule.matchGapMinutes;
  const duration = schedule.matchDurationMinutes;
  const scheduleMap = {};
...
export function buildRuntimeState(category, totals = {}, now = new Date()) {
  ...
  const scheduleMap = buildScheduleMap(normalizedCategory, baseData, now);
